...
static inline void *kcalloc(size_t n, size_t sz, gfp_t f __attribute__((unused)))
{
	return calloc(n, sz);
}
...
/* kzalloc_objs(type, n): array di n elementi di quel tipo, azzerato. */
#define kzalloc_objs(type, n) kzalloc(sizeof(type) * (n), GFP_KERNEL)
