...
Stesso sorgente della copia `wl-diag`, buildato contro il kernel 2.6.30 dello
stock firmware della DSL-3580L (vedi `Makefile` per i vincoli su KDIR,
`kallsyms_lookup_name` non esportata e opzione `klookup=`). Questa e' la
variante da usare per il radio **integrato** del 6362, perche' gira sul SoC
stesso; la copia `wl-diag` serve per un host con kernel 3.4-rt.

## Riposizionamento su N-PHY (questo repo)

Questa copia e' tarata sul radio 2.4 GHz **integrato nel BCM6362** (N-PHY rev 8,
radio 2057 rev 8), non sull'AC-PHY. Differenze rispetto alla copia in
`b43-ac-wip`, tutte verificate sul blob della board (`wl 6.30.102.7`, simboli e
prologhi letti con `mips-linux-gnu-objdump`):

- tabelle: `wlc_phy_table_{read,write}_nphy` al posto delle `_acphy`;
- aggiunti `and_radio_reg` / `or_radio_reg` (op 35/36): il ramo N-PHY del blob
  li usa, l'AC-PHY no;
- aggiunto `si_pmu_spuravoid` (op 37), che su N-PHY 2.4 GHz fa parte del cambio
  canale — con il caveat che del disasm e' verificato solo il prologo, non
  quale argomento porta il valore;
- aggiunto `phy_reg_write_array` (op 38) come **marcatore**: le sue op passano
  comunque dagli accessor gia' agganciati, come si vede nel trace (dopo ogni
  marcatore compaiono le op corrispondenti), quindi non era un buco di
  copertura; il record serve solo a delimitare gli initvals;
- lista `skipphyrd` vuota di default: il polling DFS che la motivava e' roba
  5 GHz, qui non esiste.

**Il workflow di cattura piu' sotto e' scritto per la scheda PCIe.** Per il
radio integrato non c'e' PCI da rescannare: il ciclo attach/init/up si riottiene
con `wl down; wl up` oppure scaricando e ricaricando `wl`, e il device e'
`0x14e4:0x435f` sul backplane del SoC. Il resto (pipe verso l'host, ordine di
avvio della cattura, decodifica) vale identico.

Cosa cattura ora: PHY rd/wr/mod, **PHY and/or distinti** (op 19/20), radio
rd/wr/mod (write, mod, and, or), PMU cc/rc/pll/spuravoid, GPIO ChipCommon,
tabelle nphy, e il **controllo verso
...
- sono **in ritardo di un ciclo**: nella fase 20b compaiono `ch=40`, `ch=36`,
  ...
  anche `wlc_phy_chanspec_set_nphy`.
...
e non la variante per-PHY perche' scatta per ogni PHY ed e' piccola (96B su 6.30,
...
RXIQ, PAPD e `do_dummy_tx` (per N-PHY: `wlc_phy_loadsampletable_nphy`,
`wlc_phy_runsamples_nphy`). Non era coperto
...
Nel 7.14 `noise_read_shmem` chiama la crs_min_pwr_cal della PHY, nel 6.30
...
## Workflow di cattura (target 2.4 GHz, radio integrato = `0x14e4:0x435f`)
